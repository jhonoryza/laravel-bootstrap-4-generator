...
trait ValidationTrait
{
    ...
    protected function getValidationRules(string $key, bool $isNullable): string
    {
        if (app('db')->getSchemaBuilder() instanceof PostgresBuilder) {
            return ($isNullable ? 'nullable|' : 'required|') . self::validationPostgresRules[$key];
        }
        return ($isNullable ? 'nullable|' : 'required|') . self::validationMysqlRules[$key];
    }
}
